...
func getUserIDFromCtx(ctx context.Context) (uuid.UUID, error) {
	...
	return uuid.Nil, errors.New("auth context value not set")
}
...
func (s *server) withAuth(next http.HandlerFunc) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			respondWithError(r.Context(), w, apierrors.FromErr(http.StatusUnauthorized, err))
			return
		}
...
func (s *server) handlerRefresh(w http.ResponseWriter, r *http.Request) {
	...
	if refreshToken.RevokedAt.Valid {
		respondWithError(r.Context(), w, apierrors.FromErr(http.StatusUnauthorized, errors.New("resfresh token has been revoked")))
		return
	}

	if !time.Now().Before(refreshToken.ExpiresAt) {
		respondWithError(r.Context(), w, apierrors.FromErr(http.StatusUnauthorized, errors.New("resfresh token has expired")))
		return
	}
	...
	respondWithJSON(r.Context(), w, http.StatusCreated, auth.JWTToken{Token: token})
}
...
func (s *server) handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(r.Context(), w, apierrors.FromErr(http.StatusNotFound, err))
		return
	}

	if !v {
		respondWithError(r.Context(), w, apierrors.FromErr(http.StatusNotFound, errors.New("incorrect password")))
		return
	}
	...
	respondWithJSON(r.Context(), w, http.StatusOK, auth.IdentityWithToken{
		ID:           dbIdentity.ID,
		Token:        token,
