...
SEED_PASSWORD="${SEED_PASSWORD:-seed-password-123}"
SEED_FIRST_NAME="${SEED_FIRST_NAME:-Seed}"
...
Creates a seed user and sample Kanban boards, columns, and tasks through
the HTTP API.

Environment variables:
    BASE_URL             API base URL (default: http://localhost:8080)
    SEED_EMAIL           Email used to sign up/log in the seed user
                          (default: seed@example.com)
    SEED_PASSWORD         Password used to sign up/log in the seed user
                          (default: seed-password-123)
    SEED_FIRST_NAME       First name used when signing up the seed user
                          (default: Seed)
    SEED_LAST_NAME        Last name used when signing up the seed user
                          (default: User)

Options:
  --reset             Wipe existing data (including identities) before seeding
...
for attempt in {1..30}; do
    # A plain connection (any HTTP response, even 401) means the server is up.
...
if [[ "$RESET_DATABASE" == true ]]; then
    printf 'Resetting existing data...\n'
...
signup_payload=$(jq -n \
    --arg email "$SEED_EMAIL" \
    --arg password "$SEED_PASSWORD" \
    --arg first_name "$SEED_FIRST_NAME" \
    --arg last_name "$SEED_LAST_NAME" \
...
if [[ "$signup_status" == 2* ]]; then
    printf '  Created new seed user\n'
else
    printf '  Seed user already exists, logging in instead\n'
fi

# Sign-up doesn't return a token, so log in either way to get one.
login_response=$(curl --silent --show-error --fail \
    --request POST \
    --header 'Content-Type: application/json' \
    --data "$login_payload" \
    "$BASE_URL/api/login")
token=$(printf '%s' "$login_response" | jq --raw-output --exit-status '.token')
...
for ((board_index = 0; board_index < ${#board_names[@]}; board_index++)); do
    ...
    for ((column_index = 0; column_index < ${#columns[@]}; column_index++)); do
        ...
        column_response=$(auth_curl \
            --request POST \
            --data "$column_payload" \
            "$BASE_URL/api/boards/$board_id/columns")
        ...
        for ((task_index = 0; task_index < ${#tasks[@]}; task_index++)); do
            ...
            auth_curl \
                --request POST \
                --data "$task_payload" \
                "$BASE_URL/api/boards/$board_id/columns/$column_id/tasks" >/dev/null
